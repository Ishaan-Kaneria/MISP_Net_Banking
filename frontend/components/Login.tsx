...
export function Login({ onLogin }: { onLogin: () => void }) {
  ...
  return (
    <main className="relative grid min-h-screen place-items-center overflow-hidden bg-paper p-5">
      <div className="blob h-96 w-96 gradient-brand animate-blob-float" style={{ top: "-8%", left: "-6%" }} />
      <div className="blob h-80 w-80 bg-fuchsia animate-blob-float" style={{ bottom: "-10%", right: "-4%", animationDelay: "-6s" }} />
      <section className="relative grid w-full max-w-4xl animate-rise grid-cols-1 overflow-hidden rounded-2xl bg-white shadow-pop md:grid-cols-2">
        <div className="relative flex flex-col overflow-hidden gradient-navy p-8 text-white md:p-10">
          <div className="blob h-56 w-56 gradient-brand animate-blob-float" style={{ top: "-20%", right: "-25%" }} />
          <div className="blob h-40 w-40 bg-orange animate-blob-float" style={{ bottom: "5%", left: "-15%", animationDelay: "-4s" }} />
          <div className="relative flex items-center gap-2.5 text-sm font-bold tracking-wide">
          ...
          <h1 className="relative mt-8 text-3xl font-bold leading-tight md:mt-10 md:text-4xl">
            Money that<br /><span className="gradient-text">understands life.</span>
          </h1>
          <p className="relative mt-3 max-w-sm leading-relaxed text-[#c3bfe0]">
          ...
          <div className="relative mt-auto flex flex-col gap-3 pt-8">
            {trustLines.map(line => (
              <div key={line} className="flex items-start gap-2.5 text-xs text-[#c3bfe0]">
                <CheckCircle2 size={15} className="mt-0.5 flex-none text-fuchsia" /> {line}
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="p-8 md:p-10">
          ...
          <label className="block text-sm font-semibold text-ink">Mobile number
            <div className="mt-2 flex items-stretch">
              <span className="flex items-center rounded-l-xl border border-r-0 border-border bg-paper px-3 text-sm font-bold text-muted">+91</span>
              <input value={phone} onChange={e => setPhone(e.target.value)} inputMode="numeric"
                className="block w-full rounded-r-xl border border-border bg-white px-3.5 py-3 text-sm text-ink focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary-light" />
            </div>
          </label>
          ...
          <div className="mt-2 flex justify-end">
            <span className="text-xs font-bold text-primary">Forgot PIN?</span>
          </div>
